...
整理浙江大学 Hello World 机器人队的报销材料。本入口适用于 Windows/PowerShell；Linux/macOS 使用 [SKILL.md](SKILL.md)。

安装目录为报销项目的 `.agents/skills/reimbursement/`；从项目根目录执行，`--root .` 指该目录。仓库源码须按 README 安装后再运行。日期参数 `YYYY-M-D` 使用用户指定日期，未指定则使用当前本地日期，执行前替换占位符。
...
开始流程前用 `Get-Command pdftotext,pdftoppm` 确认两个命令均可执行；任一缺失时停止并告知用户需要安装 Poppler，不自行安装系统软件。
...
```powershell
.\.venv\Scripts\python.exe .agents/skills/reimbursement/scripts/check_taxi_pairs.py --root .
```

### 3. 运行发票提取并修复字段

```powershell
.\.venv\Scripts\python.exe .agents/skills/reimbursement/scripts/super_invoice.py --root .
.\.venv\Scripts\python.exe .agents/skills/reimbursement/scripts/check_invoice_errors.py --root .
```
...
```powershell
.\.venv\Scripts\python.exe .agents/skills/reimbursement/scripts/apply_invoice_fixes.py --root .
.\.venv\Scripts\python.exe .agents/skills/reimbursement/scripts/check_invoice_errors.py --root .
```
...
```powershell
.\.venv\Scripts\python.exe .agents/skills/reimbursement/scripts/extract_trip_sheets.py --root .
```
...
```powershell
Set-Location -LiteralPath 'C:\实际的项目根目录'; & .\.venv\Scripts\python.exe .agents/skills/reimbursement/scripts/organize_expense_records.py --root .
```
...
```powershell
.\.venv\Scripts\python.exe .agents/skills/reimbursement/scripts/verify_screenshot_coverage.py --root . --update-report --issue-summary-json 报销工作文件/截图问题统计.json
```
...
```powershell
.\.venv\Scripts\python.exe .agents/skills/reimbursement/scripts/apply_match_actions.py --root . --actions "报销工作文件/<agent-name>.actions.json"
```
...
```powershell
.\.venv\Scripts\python.exe .agents/skills/reimbursement/scripts/generate_expense_record_docx.py --root .
.\.venv\Scripts\python.exe .agents/skills/reimbursement/scripts/generate_payment_record_docx.py --root .
.\.venv\Scripts\python.exe .agents/skills/reimbursement/scripts/generate_payment_explanations.py --root . --date YYYY-M-D
.\.venv\Scripts\python.exe .agents/skills/reimbursement/scripts/generate_reimbursement_xlsx.py --root .
.\.venv\Scripts\python.exe .agents/skills/reimbursement/scripts/generate_high_value_invoices.py --root .
```
...
```powershell
.\.venv\Scripts\python.exe .agents/skills/reimbursement/scripts/merge_output_pdfs.py --root .
```
