...
- `OCR缓存.json` 只用于定位候选；必须使用图像查看工具 直接查看每张原始截图，再与行程单中的服务商、乘车时间、车型和路线逐项比较。
...
- `报销工作文件/fix-trip-ambiguity.actions.json` — 本修复任务 输出的操作文件。
...
4. 使用图像查看工具 逐张打开原图，由你自行交叉判断图中可见信息：
...
除 `fix-bearing-invoice` 和主流程人工确认外，同一打车行程最多只能有一张 `支付记录` 和一张 `账单截图`。本修复任务 必须遵守：
