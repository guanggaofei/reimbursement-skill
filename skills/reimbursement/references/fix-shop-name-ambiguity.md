...
- **禁止使用 Python、终端自动识别、自动相似度、正则打分或自编脚本决定截图归属。** 不得通过其他工具或命令绕开识别限制；基础文本读取和写入指定 JSON 按主入口规则执行。
- `OCR缓存.json` 只用于定位候选；必须使用图像查看工具 直接查看每张原始截图，并依靠图中可见的店铺、商品、时间和订单信息作结论。
...
- `报销工作文件/fix-shop-name-ambiguity.actions.json` — 本修复任务 输出的操作文件。
...
4. 使用图像查看工具 逐张打开原图，由你自行比较图中可见的店铺名称和商品描述：
...
除 `fix-bearing-invoice` 和主流程人工确认外，同一材料费发票最多只能有一张 `支付记录` 和一张 `账单截图`。本修复任务 必须遵守：
