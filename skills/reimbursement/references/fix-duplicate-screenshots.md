...
- **禁止使用 Python、终端自动识别、图片哈希、自动相似度、正则打分或自编脚本决定截图归属或质量。** 不得通过其他工具或命令绕开识别限制；基础文本读取和写入指定 JSON 按主入口规则执行。
- `OCR缓存.json` 只用于定位候选；必须使用图像查看工具 直接查看每张原始截图，比较可见的店铺、服务商、金额、时间、商品、订单号和清晰度。
...
- `报销工作文件/fix-duplicate-screenshots.actions.json` — 本修复任务 输出的操作文件。

## 步骤

1. 按冲突发票/金额对图片分组。
2. 对每张图片，从 `OCR缓存.json` 读取 OCR 文本。
3. 使用图像查看工具 逐张打开原图，由你自行比较图中可见的店铺名称、金额、日期时间、商品描述、订单号，判断是否为同一笔交易。
4. 如果确认为重复：
...
除 `fix-bearing-invoice` 和主流程人工确认外，同一材料费发票、同一打车行程最多只能有一张 `支付记录` 和一张 `账单截图`。本修复任务 是唯一性兜底修复器：
