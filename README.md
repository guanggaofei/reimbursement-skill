...
> 当前版本：opencode。Codex 主版本请切换到 `codex`；Claude Code 版本请切换到 `claude-code`。
>
...
>
> 安装前先选择对应分支，使用该分支 README 中的命令。下方保留已有两种入口的说明；安装 Claude Code 时，克隆命令中的分支名改为 `claude-code`。
...
- Agent 获取本仓库时，必须将 Git 仓库克隆到 `/tmp/reimbursement-opencode-skill`，不得克隆到用户的报销项目目录或其他长期保存目录：
  ...
  ```bash
  git clone --branch opencode https://github.com/guanggaofei/reimbursement-opencode-skill.git /tmp/reimbursement-opencode-skill
  ```
